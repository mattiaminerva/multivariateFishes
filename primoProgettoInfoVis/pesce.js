...
function isNear(centerX,centerY,nearsX,nearsY){
	...
	while(i<=nearsX.length){
		if( Math.sqrt( Math.pow(nearsX[i]-centerX,2)+(Math.pow(nearsY[i]-centerY,2)) )  < 250 ){
			return true;
		}
		i++;
	}
...
function center(){
	var centerX = d3.randomUniform(margin.left+80,width-margin.right-80)();
	var centerY = d3.randomUniform(margin.bottom,height-margin.top)();
...
function drawFish(dataset,i,color,dom,centerX,centerY){
	...
	setInterval(function(){
		...
		//	oppure se si raggiunge la dimensione massima
			...
			.attr("class", "fish_"+i)
			.attr("id",i)
			.attr("mouseover","details(event)")
			.attr("onclick","modifyFishes(event)")
			.attr("points",""+(centerX-th-b/6)+","+(centerY)+" "+(centerX-th-tw-padTail)+","+(centerY-th)+" "+(centerX-th-tw-padTail)+","+(centerY+th)+"")
			.attr("fill","red")
			.attr("stroke","red")
